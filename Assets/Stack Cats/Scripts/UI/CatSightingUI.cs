...
namespace Tofuwu.StackCats.UI
{
    ...
    public delegate void BondingProgressStateChanged(CatSightingUI catSightingUI, bool isEntering);
    ...
    public class CatSightingUI : MonoBehaviour
    {
        ...
        public static event BondingProgressStateChanged OnBondingProgressStateChanged;
        ...
        public CatSightingUIState State
        {
            get { return _state; }
        }
        ...
        {
            if (_state == CatSightingUIState.AllInfoDisplayed)
                return false;
            ...
            switch (_state)
            {
                ...
                case CatSightingUIState.DisplayingRewards:
                    ChangeState(
                        _sightingsCount == Cat.BondedAt
                            ? CatSightingUIState.DisplayingBondedEvent
                            : CatSightingUIState.AllInfoDisplayed
                    );
                    break;
        ...
        protected void Start()
        {
            ...
            else
            {
                ...
                if (CatPortraitImage)
                {
                    ...
                    CatPortraitImage.color = _isAlreadyBonded
                        ? Color.white
                        : NotBondedPortraitColor;
                }
                if (CatNameText)
                    CatNameText.text = _isAlreadyBonded ? Cat.Name : "Name: ???";
                if (NewCatLabel)
                    NewCatLabel.gameObject.SetActive(_sightingsCount == 0);
                if (PresentReceivedRectTransform)
                    PresentReceivedRectTransform.gameObject.SetActive(false);
                if (UncommonBorder)
                    UncommonBorder.SetActive(Cat.Rarity == Rarity.Uncommon);
                if (RareBorder)
                    RareBorder.SetActive(Cat.Rarity == Rarity.Rare);

                if (_isAlreadyBonded)
                {
                    // Hide irrelevant UI stuff
                    if (BondingGuageRectTransform)
                        BondingGuageRectTransform.gameObject.SetActive(false);
                    if (PresentIconsRectTransform)
                        PresentIconsRectTransform.gameObject.SetActive(false);
                    ChangeState(CatSightingUIState.AllInfoDisplayed);
                    ...
                    // Background stuff
                    if (BackgroundImage)
                        BackgroundImage.color = BondedColor;
                    if (ScrollingBackgroundImage)
                        ScrollingBackgroundImage.sprite = BondedSprite;
                }
                else
                {
                    BondingGuageMaskFillTransform.Amount = Mathf.Clamp(
                        (_sightingsCount - 1) / (float)Cat.BondedAt,
                        0.0f,
                        1.0f
                    );
                    ...
                    {
                        float presentIconPositionTransform =
                            reward.AtSightingCount / (float)Cat.BondedAt;
                        ...
                        presentIcon.rectTransform.anchorMin = new Vector2(
                            presentIconPositionTransform,
                            0.5f
                        );
                        presentIcon.rectTransform.anchorMax = new Vector2(
                            presentIconPositionTransform,
                            0.5f
                        );
                        presentIcon.rectTransform.sizeDelta = Vector2.one * 64;
                        presentIcon.sprite =
                            _sightingsCount > reward.AtSightingCount
                                ? OpenedPresentIcon
                                : UnopenedPresentIcon;
        ...
        private void ChangeState(CatSightingUIState state)
        {
            ...
            switch (_state)
            {
                case CatSightingUIState.NotStarted:
                    break;
                case CatSightingUIState.DisplayingBondingProgress:
                    BondingGuageMaskFillTransform.SetAmountImmediate(
                        Mathf.Clamp(_sightingsCount / (float)Cat.BondedAt, 0.0f, 1.0f)
                    );
                    OnBondingProgressStateChanged?.Invoke(this, false);
                    break;
                case CatSightingUIState.DisplayingRewards:
                    break;
            }
            ...
            switch (_state)
            {
                case CatSightingUIState.NotStarted:
                    break;
                case CatSightingUIState.DisplayingBondingProgress:
                    OnBondingProgressStateChanged?.Invoke(this, true);
                    BondingGuageMaskFillTransform.Amount = Mathf.Clamp(
                        _sightingsCount / (float)Cat.BondedAt,
                        0.0f,
                        1.0f
                    );
                    break;
                case CatSightingUIState.DisplayingRewards:
                    if (_presentIcons.ContainsKey(_sightingsCount))
                    {
                        ...
                        if (PresentReceivedRectTransform)
                        {
                            ...
                            LeanTween
                                .scale(
                                    PresentReceivedRectTransform.gameObject,
                                    Vector3.one * 1.5f,
                                    0.5f
                                )
                ...
                case CatSightingUIState.DisplayingBondedEvent:
                    if (CatPortraitImage)
                        CatPortraitImage.color = Color.white;
                    if (BondingGuageRectTransform)
                        BondingGuageRectTransform.gameObject.SetActive(false);
                    if (PresentIconsRectTransform)
                        PresentIconsRectTransform.gameObject.SetActive(false);
                    ...
                    if (BondingEventFaderCanvasGroup)
                    {
                        ...
                        LeanTween
                            .alphaCanvas(BondingEventFaderCanvasGroup, 0.0f, 2.0f)
