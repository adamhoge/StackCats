...
namespace Tofuwu.StackCats
{
    public static class GalaxyFlavoredPuzzleBuilder
    {
        ...
        /// <returns></returns>
        public static GalaxyFlavoredPuzzle BuildFromModel(GalaxyFlavoredPuzzleModel puzzleModel, GalaxyFlavoredPuzzle puzzlePrefab, CatManager catManager)
        {
            ...
            puzzle.RaiseStacksMovesRemaining = puzzle.RaiseStacksInterval - puzzle.NumMovesMade % puzzle.RaiseStacksInterval;
            ...
            foreach (StackModel stackModel in puzzleModel.Stacks)
            {
                ...
                foreach (BlockModel blockModel in stackModel.Blocks)
                {
                    ...
                    {
                        GalaxyBlock galaxyBlock = puzzle.CreateGalaxyBlock(blockModel.GalaxyBlock.PrimaryNumber, blockModel.GalaxyBlock.SecondaryNumber);
        ...
        /// <returns></returns>
        public static GalaxyFlavoredPuzzle GenerateNew(PuzzleArea puzzleArea, int numStacks, int maxBlocks = 10, int difficulty = 1)
        {
            difficulty = 12;

            // Create the empty puzzle.
            GalaxyFlavoredPuzzle puzzle = Object.Instantiate((GalaxyFlavoredPuzzle)puzzleArea.PuzzlePrefab);
            puzzle.name = "Galaxy Puzzle";
            puzzle.MaxStackHeight = maxBlocks;
            for (int i = 0; i < numStacks; i++) puzzle.AddNewStack();
            ...
            int numBlocks = baseNumBlocks + Random.Range(0, rangedNumBlocks) + (int)((float)difficulty / 20 * difficultyNumBlocks);
            ...
            int maxSumBlockValueChange = Mathf.FloorToInt((float)numSumBlocks / 2 * (numSumBlocks <= 2 ? 2 : 1));
            ...
            foreach (Stack stack in puzzle.Stacks)
            {
                for (int i = 0; i < (stack == lowStack ? lowStackSize : stackSize); i++)
                {
                    ...
                    {
                        PuzzleBuilder.AddRandomPuzzleBlockToTop(puzzle, stack, minPuzzleBlockValue, maxPuzzleBlockValue);
                    }
                    else if (Random.value > 0.4f)
                    {
                        AddRandomGalaxyBlockToTop(puzzle, stack, minPuzzleBlockValue, maxPuzzleBlockValue);
                    }
                }
            }
            ...
            for (int i = 0; i < numSumBlocks; i++)
            {
                ...
                while (sumBlockIndices.Contains(newSumBlockIndex)) newSumBlockIndex = Random.Range(0, numSumAndCatBlocks);
            ...
            while (blocksAdded < numSumAndCatBlocks && padding >= 0)
            {
                ...
                List<Stack> stacksByMovableCountRange = PuzzleBuilder.GetStacksByMovableCountRange(puzzle, 0, padding);
                if (stacksByMovableCountRange.Count > 0)
                {
                    int minStackSize = stacksByMovableCountRange.Min(s => s.Blocks.Count);
                    destinationStack = PuzzleBuilder.GetRandomStackByCountRange(stacksByMovableCountRange, minStackSize, minStackSize + padding);
                }
                if (!destinationStack)
                {
                    int minStackSize = puzzle.Stacks.Min(s => s.Blocks.Count);
                    int maxStackSize = minStackSize + padding;
                    if (maxStackSize >= maxBlocks - 3) maxStackSize = maxBlocks - 3;
                    destinationStack = PuzzleBuilder.GetRandomStackByCountRange(puzzle.Stacks, minStackSize, maxStackSize);
                    if (!destinationStack) break;
                }

                int spaceAvailable = destinationStack.MaxBlocks - destinationStack.Blocks.Count - 1;
                int maxMovableBlocks = puzzle.Stacks.Max(s => PuzzleBuilder.GetNumMovableBlocks(puzzle, s));
                if (maxMovableBlocks > spaceAvailable) maxMovableBlocks = spaceAvailable;
                if (maxMovableBlocks < 1) break;

                int usablePadding = padding >= maxMovableBlocks ? maxMovableBlocks - 1 : padding;
                List<Stack> potentialSourceStacks = PuzzleBuilder.GetStacksByMovableCountRange(puzzle, 1, maxBlocks, new List<Stack> { destinationStack });
                Stack sourceStack = potentialSourceStacks.Count > 0 ? potentialSourceStacks[Random.Range(0, potentialSourceStacks.Count)] : null;
                if (!sourceStack) break;

                int numMovableSourceBlocks = PuzzleBuilder.GetNumMovableBlocks(puzzle, sourceStack);
                if (numMovableSourceBlocks > maxMovableBlocks) numMovableSourceBlocks = maxMovableBlocks;
                int minIndex = sourceStack.Blocks.Count - numMovableSourceBlocks;
                int maxIndex = minIndex + usablePadding;
                if (maxIndex > sourceStack.Blocks.Count) maxIndex = sourceStack.Blocks.Count;
                Block sourceBlock = sourceStack.Blocks[Random.Range(minIndex, maxIndex)];

                bool blockAdded;
                if (sumBlockIndices.Contains(blocksAdded))
                {
                    int sumBlockValue = (numSumBlocks <= 2 ? 2 : 1) * (Random.value >= .5f ? 1 : -1);

                    blockAdded = PuzzleBuilder.AddMoveSumBlock(puzzle, sumBlockValue, sourceStack, sourceBlock, destinationStack);
                }
                else
                {
                    blockAdded = PuzzleBuilder.AddMoveCatBlock(puzzle, sourceStack, sourceBlock, destinationStack);
                }
        ...
        public static bool AddRandomGalaxyBlockToTop(GalaxyFlavoredPuzzle puzzle, Stack stack, int min = 1, int max = 9)
        {
        ...
        /// <returns></returns>
        public static GalaxyBlock CreateAdjacentGalaxyBlock(GalaxyFlavoredPuzzle puzzle, PuzzleBlock puzzleBlock, int min = 1, int max = 9)
        {
            int primaryNumber;
            int secondaryNumber;
            if (puzzleBlock)
            {
                ...
                incrementPrimary = puzzleBlock.PrimaryNumber == min || Random.Range(0, 2) == 0 && puzzleBlock.PrimaryNumber < max;
